...
**Architecture:** `rebar_insert.scad` остаётся источником геометрии втулки и нового режима `driver`; независимая от `Fit` крышка Ø11,8 мм добавляется после существующей рабочей длины, не меняя спираль и вход. Python-тесты проверяют диагностические размеры, зазор крышки в фактическом канале Ø12,2 мм, реальные Manifold STL и торцевые пробы. `scripts/rebar_cut_list.py` сохраняет точные внутренние вычисления, но применяет новый отступ упора и формирует целочисленную ведомость.
...
- `Cap_Thickness_mm = 0.5`, `Cap_D_mm = 11.8`; крышка имеет 0,4 мм диаметрального запаса в канале 12,2 мм, печатается сверху мостом, втулка остаётся буртиком на столе.
...
- Produces: `Cap_Thickness_mm`, `Cap_D_mm`, `_driverInnerDiameter()`, `driverTool()`, режим `Part="driver"`, пробы крышки, её нахлёста и сквозного отверстия трубки.

- [ ] **Step 1: Написать падающие тесты крышки**

В диагностике ожидать `cap_t=0.5`, `cap_d=11.8` и зазор 0,4 мм относительно
`Hole_D_mm=12.2` из фактической модели крепления. Изменить ожидаемую высоту полной детали на
...
крышки и 0 non-manifold рёбер. Дополнительная пара проб подтверждает, что
винтовые прорези открыты внутри 0,1-мм соединительного нахлёста.
...
Пара осевых проб около обоих торцов подтверждает, что отверстие трубки сквозное.
...
наружный `union()`. Сплошной диск использует `Cap_D_mm`, а не `cageDiameter`.
Скрытый модуль `_capJoinBands()` заходит на 0,1 мм в существующие ленты и
повторяет центральное и винтовые вычитания, чтобы не сдвигать упор арматуры.
Ключевая часть модуля принимает вид:
...
Проверить `Cap_Thickness_mm > 0`, `Cap_D_mm > _coreBoreDiameter()`,
`Cap_D_mm <= _cageDiameter("vloose")`, обе суммы рабочей длины и толщины крышки
относительно `Socket_Depth_mm`, `Driver_Diametral_Clearance_mm > 0`,
